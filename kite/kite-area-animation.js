...
function initAnimation() {
  ...
  function createRotateTranslateAnimation(triangleVertices, T2_index, T3_index, K2_vertex, K3_vertex, color) {
    ...
    const translatedVertices = rotatedVertices.map(v => ({ x: v.x + dxTranslate, y: v.y + dyTranslate }));
    const triVec = { x: translatedVertices[T3_index].x - translatedVertices[T2_index].x, y: translatedVertices[T3_index].y - translatedVertices[T2_index].y };
    const kiteVec = { x: K3_vertex.x - K2_vertex.x, y: K3_vertex.y - K2_vertex.y };
    ...
    const finalVertices = translatedVertices.map(v => {
      ...
      return { x: K2_vertex.x + dx * cos - dy * sin, y: K2_vertex.y + dx * sin + dy * cos };
    });
    const finalCentroid = { x: (finalVertices[0].x + finalVertices[1].x + finalVertices[2].x)/3, y: (finalVertices[0].y + finalVertices[1].y + finalVertices[2].y)/3 };
    const translateAnim = createTranslatePolygonAnimation(ctx, { points: rotatedVertices, centroid: getTriangleCentroid(rotatedVertices) }, getTriangleCentroid(rotatedVertices), finalCentroid, duration, { strokeStyle: color, lineWidth: 1, alpha: 1 });
  ...
  triangleMappings.forEach((mapping, i) => {
    const { rotateAnim, translateAnim } = createRotateTranslateAnimation(triangles[i].vertices, mapping.T2, mapping.T3, mapping.K2, mapping.K3, mapping.color);
  ...
  const completedMap = [
    ...
    [0,5,7,9], [0,5,7,9], [0,7,9,11], [0,7,9,11], [0,7,9,11,13]
  ];
